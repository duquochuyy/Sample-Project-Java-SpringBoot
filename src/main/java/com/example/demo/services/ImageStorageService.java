...
public class ImageStorageService implements IStorageService {
    ...
    private boolean isImageFile(MultipartFile file) {
        ...
        return Arrays.asList(new String[] { "jpg", "jpeg", "png", "gif" }).contains(fileExtention.trim().toLowerCase());
    }
    ...
    public String storageFile(MultipartFile file) {
        try {
            ...
            // check size file <= 5mb
            float fileSizeInMegabytes = file.getSize() / (1024 * 1024);
            if (fileSizeInMegabytes > 5.0) {
                throw new RuntimeException("File size is too large. File size <= 5MB");
            }
            ...
            // sao chép dữ liệu file từ file nhận vào file destination
            ...
            throw new RuntimeException("Could not store file " + file.getOriginalFilename() + ". Please try again!", exception);
        }
    }
